...
import com.almufeed.technician.business.domain.utils.hideKeyboard
import com.almufeed.technician.business.domain.utils.isOnline
import com.almufeed.technician.business.domain.utils.toast
import com.almufeed.technician.databinding.ActivityTaskBinding
...
import com.almufeed.technician.ui.home.adapter.TaskRecyclerAdapter
import com.almufeed.technician.ui.home.adapter.TaskRecyclerAdapterOffline
...
class TaskActivity : AppCompatActivity(), BaseInterface {
    ...
    private lateinit var dataTaskList: ArrayList<BookData>
    ...
    private fun filter(text: String,taskListResponse: ArrayList<BookData>) {
        // creating a new array list to filter our data.
        val filteredlist = ArrayList<BookData>()

        // running a for loop to compare elements.
        for (item in taskListResponse) {
            // checking if the entered string matched with any item of our recycler view.
            if (item.TaskId?.lowercase()?.contains(text.lowercase(Locale.getDefault()))!!) {
    ...
    private var watcher = object : TextWatcher {
        ...
        override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
            filter(s.toString(),dataTaskList)
                ...
                when (dataState) {
                    ...
                    is DataState.Success -> {
                        ...
                        if(dataState.data.task.isEmpty()){
                            ...
                            dataTaskList = dataState.data.task
...
// If needed, you can handle the remaining elements in the larger list separately
    ...
    override fun showNetworkSnackBar(isNetworkAvailable: Boolean) {
        if (isNetworkAvailable) {
            if (snack.isShown) {
                this.toast(getString(com.almufeed.technician.R.string.text_network_connected))
            }
        } else {
            snack.show()
        }
